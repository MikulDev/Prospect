...
public class TemplateEditorWindow extends View {
    ...
    private void initialize() {
        ...
        if (Platform.isAndroid() || Platform.isIOS()) {
            ScrollPane mobileScroll = new ScrollPane(mainLayout);
    ...
    private Node createElementCard(AbstractElementTemplate element, int index) {
        ...
        upButton.setOnAction(e -> moveElement(index, -1));
        upButton.setDisable(index == 0);

        // Move down button
        Button downButton = new Button();
        downButton.setGraphic(MaterialDesignIcon.ARROW_DOWNWARD.graphic());
        downButton.setOnAction(e -> moveElement(index, 1));
        downButton.setDisable(index == template.getElements().size() - 1);

        // Delete button
        Button deleteButton = new Button();
        deleteButton.setGraphic(MaterialDesignIcon.DELETE.graphic());
        deleteButton.setOnAction(e -> removeElement(element));
